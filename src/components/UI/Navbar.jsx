...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 flex justify-between items-center py-2.5">
            ...
            {isAuthenticated ? (
              <div ref={profileRef} className="relative">
                ...
                >
                  ...
                  <span className="text-primary font-medium">Profil</span>
                </button>
                {isProfileOpen && (
              ...
              <>
                <button onClick={() => setModalOpen(true)} className="px-4 py-2 rounded-md hover:bg-amber-800 hover:text-white">
                  Masuk
                </button>
      ...
      {isMenuOpen && (
        <div ref={menuRef} className="fixed top-16 left-0 w-full bg-white shadow-md p-6 flex flex-col space-y-4 z-50">
          <Link to="/" className="text-primary font-medium hover:text-secondary transition-colors duration-300">Beranda</Link>
          <Link to="/about" className="text-primary font-medium hover:text-secondary transition-colors duration-300">Tentang</Link>
          <Link to="#" className="text-primary font-medium hover:text-secondary transition-colors duration-300">Kontak</Link>
          {isAuthenticated ? (
            <>
              <Link to="/dashboard" className="text-primary font-medium hover:text-secondary transition-colors duration-300">Dashboard</Link>
              <Link to="/profile/edit" className="text-primary font-medium hover:text-secondary transition-colors duration-300">Profil Saya</Link>
              <button onClick={() => { localStorage.removeItem("token"); setIsAuthenticated(false); }} className="w-full border px-4 py-2 rounded-md hover:bg-red-600 hover:text-white">
                Keluar
              </button>
            </>
          ) : (
